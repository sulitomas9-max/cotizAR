...
async function actualizarDesdePDF() {
  ...
  try {
    ...
    // Extraer alquiler promedio CABA del PDF
    // El reporte tiene: "monoambiente medio de 40 m2 se alquila por $ XXX.XXX"
    const alqMonoMatch = text.match(/monoambiente[^$]*\$\s*([\d.,]+)\s*por mes/i) ||
                         text.match(/40\s*m2[^$]*\$\s*([\d.,]+)/i);
    const alq2ambMatch = text.match(/2 ambientes[^$]*\$\s*([\d.,]+)\s*por mes/i) ||
                         text.match(/50\s*m2[^$]*\$\s*([\d.,]+)/i);

    if (alqMonoMatch && alq2ambMatch) {
      const alqMono = parseFloat(alqMonoMatch[1].replace(/\./g,'').replace(',','.'));
      const alq2amb = parseFloat(alq2ambMatch[1].replace(/\./g,'').replace(',','.'));
      if (alqMono > 100000 && alq2amb > 100000) {
        // Precio ponderado por m²: (mono/40 + 2amb/50) / 2
        const nuevoAlqM2 = Math.round((alqMono/40 + alq2amb/50) / 2);
        if (nuevoAlqM2 > 5000 && nuevoAlqM2 < 100000) {
          global.ALQ_BASE_CABA_M2_MES_ACTUALIZADO = nuevoAlqM2;
          console.log(`[PDF-UPDATE] Alquiler base CABA: $${nuevoAlqM2}/m²/mes`);
        }
      }
    }
...
app.get('/api/cotizar', async (req, res) => {
  ...
  // Alquiler estimado — Feb 2026 (ZonaProp Index)
  // Base CABA: monoamb 40m² = $693.960/mes → $17.349/m²/mes
  //            2 amb   50m² = $814.400/mes → $16.288/m²/mes  
  //            3 amb   70m² = $1.109.000/mes → $15.843/m²/mes
  // Promedio ponderado CABA: ~$16.200/m²/mes
  // Escalamos por zona usando el ratio precio_venta_barrio / precio_venta_promedio_CABA
  const ALQ_BASE_CABA_M2_MES = global.ALQ_BASE_CABA_M2_MES_ACTUALIZADO || 16200;
  const PROMEDIO_VENTA_CABA  = 2455;  // USD/m² promedio CABA Feb 2026

  // Factor zonal: barrios caros tienen alquileres proporcionalmente más altos
  const factorZonal = b.m2_mediana / PROMEDIO_VENTA_CABA;
  const alqBaseM2Mes = Math.round(ALQ_BASE_CABA_M2_MES * factorZonal);

  // Si hay dato scrapeado en DB, usarlo; si no, usar el calculado
  ...
  // Aplicar factores de ajuste (antigüedad y amenities impactan también en el alquiler)
