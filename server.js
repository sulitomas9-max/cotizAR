...
async function actualizarDesdePDF() {
  ...
  try {
    ...
    // Extraer precios por barrio
    const barriosExtraidos = {};
    const barrioBuscar = [
      { key: 'puerto_madero', nombre: 'Puerto Madero' },
      { key: 'palermo',       nombre: 'Palermo'       },
      { key: 'belgrano',      nombre: 'Belgrano'      },
      { key: 'nuñez',         nombre: 'Núñez'         },
      { key: 'recoleta',      nombre: 'Recoleta'      },
      { key: 'lugano',        nombre: 'Lugano'         },
      { key: 'nueva_pompeya', nombre: 'Nueva Pompeya' },
      { key: 'la_boca',       nombre: 'La Boca'       },
    ];

    barrioBuscar.forEach(b => {
      const regex = new RegExp(b.nombre + '[\\s\\S]{0,50}?([\\d]{1,4}[.,][\\d]{3})\\s*(?:USD|$)', 'i');
      const match = text.match(regex);
      if (match) {
        const precio = parseFloat(match[1].replace(/\./g,'').replace(',','.'));
        if (precio > 500 && precio < 10000) {
          barriosExtraidos[b.key] = precio;
          console.log(`[PDF-UPDATE] ${b.nombre}: USD ${precio}/m²`);
        }
      }
    });
